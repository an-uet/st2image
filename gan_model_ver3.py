...
from keras.layers import Input, ReLU, LeakyReLU, Dense, Activation, Reshape, BatchNormalization, Conv2D, Conv2DTranspose, Flatten
...
def train(n_filter=256, z_dim=100, lr_d=2e-4, lr_g=2e-4, epochs=300, batch_size=128,
    ...
    plot_images(np.array(random_images), 'fake_image_gpu/real_image.png')
    ...
    for e in tqdm(range(1, epochs + 1)):
        ...
        for i in range(n_steps):
            ...
            if i == 0 and e % verbose == 0:
                ...
                plot_images(fake_images, "fake_image_gpu/fake_images_e_{}.png".format(e))
                plot_losses(losses_d, losses_g, "fake_image_gpu/losses.png")
                # losses = pd.DataFrame(losses, columns=['loss_d', 'loss_g'])
                # losses.to_csv('fake_image_gpu/train_loss.csv')
        ...
        plot_losses(losses_d_epoch, losses_g_epoch, "fake_image_gpu/losses_epoch.png")

        if e % 30 == 0:
            # Save the model checkpoint
            G.save("fake_image_gpu/model/G_model_epoch_{}.h5".format(e))
            D.save("fake_image_gpu/model/D_model_epoch_{}.h5".format(e))
